...
indication_mappings = {}
cmpd_mappings = {}
required_parameters = {"search_compound": ["cmpd_name"],
                       "similar_compounds": ["cmpd_id"],
                       "canpredict_compounds": ["indication"],
                       "canpredict_indications": ["cmpd_id"],
                       "canpredict_adr": ["cmpd "],
                       "canbenchmark": [],
...
@app.route("/")
def home():
    return render_template('cando.html', home_selection="current")
...
def cando():
    ...
    #    selected_method = ""
    selected_method = "search_compound"
...
def compounds_drugs():
    ...
    #    selected_method = ""
    selected_method = "similar_compounds"
...
def indications():
    ...
    if not selected_method in cando_obj.indications_methods():
        selected_method = ""
    selected_method = "canpredict_indications"
...
@app.route("/search_compound", methods=['POST'])
def get_compound():
    global file_name
    global b
    body = dict(request.form)
    cmpd_name = body['cmpd_name'].strip()
    n=5
    file_name = "outputs/cando/search_compound.txt"
    if 'n' in body.keys() and len(body['n']) > 0:
        n = int(body['n'])
    if 'save' in body.keys() and len(body['save']) > 0:
        file_name = int(body['save'])
    cando_obj.search_compound(cmpd_name, n=n, save=file_name) 
    #with open(file_name, 'w') as f:
    #    f.write(f"name\tid\tstatus\tindication")
    #    f.write("\n")
    #    f.write(f"{cmpd.name}\t{cmpd.id_}\t{cmpd.status}\t{cmpd.indications[0].name}")
    #f.close()
    b = True
    return redirect("/cando")

@app.route("/similar_compounds", methods=['POST'])
def similar_compounds():
    global file_name
    global b
    body = dict(request.form)
    cmpd_id = int(body['cmpd_id'].strip())
    n=10
    file_name = "outputs/compounds_drugs/similar_compounds.txt"
    if 'n' in body.keys() and len(body['n']) > 0:
        n = int(body['n'])
    if 'save' in body.keys() and len(body['save']) > 0:
        file_name = int(body['save'])
    cando_obj.similar_compounds(cando_obj.get_compound(cmpd_id), n=n) 
    #with open(file_name, 'w') as f:
    #    f.write(f"name\tid\tstatus\tindication")
    #    f.write("\n")
    #    f.write(f"{cmpd.name}\t{cmpd.id_}\t{cmpd.status}\t{cmpd.indications[0].name}")
