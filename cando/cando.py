...
class CANDO(object):
    ...
    def search_compound(self, cmpd_name, n=5, save=''):
        """!
        ...
        cmpd_name = cmpd_name.strip().lower().replace(' ', '_')
        cando_drugs = list(map(return_names, self.compounds))
        nms = difflib.get_close_matches(cmpd_name, cando_drugs, n=n, cutoff=0.5)
        with open(save, 'w') as f:
            f.write('id\tname')
            f.write('\n')
            for nm in nms:
                f.write("{}\t{}".format(id_d[nm], nm))
                f.write('\n')
        f.close()
    ...
    def canpredict_indications(self, cmpd_id, n=10, topX=10, consensus=True, sorting='prob', save=''):
        cmpd=cmpd_id
        """!
    ...
    def similar_compounds(self, cmpd, n=10):
        ...
        with open(file_name, 'w') as f:
            f.write("rank\tdist\tid\tname")
            f.write("\n")
            for i in range(n):
                f.write("{}\t{:.3f}\t{}\t{}".format(i+1, cmpd.similar[i][1], cmpd.similar[i][0].id_, cmpd.similar[i][0].name))
                f.write("\n")
            f.close()
            return
